...
class WGAN(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('discriminator') as scope:
            _, self.d_x, = self._create_discriminator(self.x, self.hidden_size)
            scope.reuse_variables()
            _, self.d_g, = self._create_discriminator(self.g, self.hidden_size)

        # Define the loss for discriminator and generator networks (see the original
        # paper for details), and create optimizers for both
        self.d_loss = - tf.reduce_mean(self.d_x) + tf.reduce_mean(self.d_g)
        ...
        self.g_opt = self._create_optimizer(self.g_loss, self.g_params,
                                            self.learning_rate)
        # weight clipping
        self.clip_D = [p.assign(tf.clip_by_value(p, -0.02, 0.02)) for p in d_vars]
    ...
    def _create_discriminator(self, input, h_dim):
        ...
        out_logits = linear(hidden, 1, scope='d_out')
    ...
    def fit(self):
        ...
        while self.epoch < self.num_epochs:
            ...
            d_x, d_loss, _, _ = self.tf_session.run(
                [self.d_x, self.d_loss, self.d_opt, self.clip_D],
